...
사용자가 특정 메시지와 함께 KLAY를 전송하고자 할 때 TxTypeValueTransferMemo를 사용합니다. TxTypeValueTransferMemo는 `to`가 외부 소유 계정인 경우에만 허용됩니다. 스마트 컨트랙트 계정으로 KLAY를 전송하려면, 대신 [TxTypeSmartContractExecution](#txtypesmartcontractexecution)을 사용하세요. 이 트랜잭션 유형에 따라 다음과 같은 변경이 이루어집니다.
