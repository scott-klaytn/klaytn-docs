...
3. `spender`의 잔액은 남은 트랜잭션 수수료만큼 감소합니다. 예: `feeRatio`가 30이면 수수료의 30%는 수수료 납부자가 지불하고, 나머지 70%는 발신자가 지불합니다.
